...
class ImageService {
  ...
  private async fetchDiverseImages(
    ...
    // Smart query generation based on location characteristics
    ...
    for (let i = 0; i < Math.min(3, shuffledQueries.length); i++) {
      apiCalls.push(
        this.searchUnsplashImages(shuffledQueries[i], page, Math.ceil(resultsPerSource / 2)).catch((error) => {
          console.warn(`Unsplash query "${shuffledQueries[i]}" failed:`, error)
          return []
        }),
      )
    }
    ...
    // Pexels calls
    for (let i = 3; i < Math.min(6, shuffledQueries.length); i++) {
      apiCalls.push(
        this.searchPexelsImages(shuffledQueries[i] || shuffledQueries[0], page, Math.ceil(resultsPerSource / 2)).catch(
          (error) => {
            console.warn(`Pexels query "${shuffledQueries[i] || shuffledQueries[0]}" failed:`, error)
            return []
          },
        ),
      )
  ...
  private generateSmartQueries(location: {
    ...
    const baseQueries = [
      `${location.city} photography`,
      `${location.city} landscape`,
      `${location.city} golden hour`,
      `${location.city} cityscape`,
      `${location.city} architecture`,
      `${location.city} sunrise`,
      `${location.city} sunset`,
      `${location.city} nature`,
      `${location.city} urban`,
      `${location.city} street photography`,
    ]

    // Add country-specific queries for variety
    const countryQueries = [
      `${location.country} landscape`,
      `${location.country} travel photography`,
      `${location.country} scenic`,
      `${location.country} culture`,
    ]

    // Add state queries if available
    const stateQueries = location.state
      ? [`${location.state} photography`, `${location.state} landscape`, `${location.state} nature`]
      : []

    // Time-based queries for golden hour app
    const timeQueries = [
    ...
    return [...baseQueries, ...countryQueries, ...stateQueries, ...timeQueries]
  }
